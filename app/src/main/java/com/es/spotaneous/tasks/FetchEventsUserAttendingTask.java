 ...
 */
public class FetchEventsUserAttendingTask extends AsyncTask<String,Void,ArrayList<Events>> {

    private static final int NUMBER_RESULTS = 5000;
    private Context mContext;

    public FetchEventsUserAttendingTask(Context context){
        mContext = context;
    }

    @Override
    protected ArrayList<Events> doInBackground(String... strings) {
        String urlString =  Server.SERVER_ADDRESS+ "/"+Server.SERVER_USER_EVENT_ATTENDING+ "/?user_id=" + strings[0];
        Log.d("FetchEventsUserAtten", urlString);
